...
Accurate MC simulations are crucial, as they serve as the foundation for hypothesis tests that compare physics models with collision data.
...
Jet formation in particle physics bridges two distinct regimes: on one end, high-energy partons emerge from the interaction, where the small QCD coupling constant allows their dynamics to be described via perturbative QCD.
As these partons evolve, they transition into a collection of frozen-out colour-singlet hadrons, leptons, and photons, interacting with the detector material at an energy scale where QCD can no longer be treated perturbatively.
Importantly, there is no first-principles physical theory that fully describes the complete evolution of a jet from its initial high-energy state to its final detection, requiring phenomenological models and empirical approaches to connect these regimes.

Parton showers are iteratively simulated using a Markovian algorithm that stochastically transitions an $n$-parton state to an $(n+1)$-parton state.
...
Crucial to the study of large-radius jets are the substructure variables $\tau_{21}$, $\tau_{32}$, and $\Dtwo$, described in \Cref{sec:fat_jets}\footnote{These variables are already normalized by the jet's total $\pt$, thus relative versions are unnecessary.}.
...
The difference between the generated point cloud kinematics and the target conditions for mass and \pt is examined, as shown in \Cref{fig:obedience}.
