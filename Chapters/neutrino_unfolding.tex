...
Monte Carlo (MC) generators use theoretical assumptions to approximate $p_\text{gen} \approx p_\text{true}$.
...
\begin{equation}
    \int p_\text{sim}(\y|\x) p_\text{gen}(\x) \diff\x = p_\text{MC}(\y) \approx p_\text{data}(\y).
\end{equation}
...
Furthermore, this only constrains the total transverse momentum, and for events producing multiple neutrinos, individual kinematics are under-constrained, leading to a continuum of possible solutions.
...
The datasets used in this study comprises simulated \ttbar events where exactly one top quark decays into a $b$-jet and a leptonically decaying $W^\pm$ boson, resulting in final states with $(e,\nu_e)$, $(\mu,\nu_\mu)$, or their antiparticles.
...
Negligible differences are observed in the reconstructed neutrino kinematics, though the minor offsets can clearly be seen for the reconstructed \Wboson and top quark invariant masses.
...
\begin{figure*}[hbtp]
    ...
    \caption{These plots test the sensitivity of \vvflows when inference is performed on a sample generated with a different top quark mass. The left plot shows the reconstructed top quark mass when neutrinos are completely neglected and while there are deviations from the nominal distribution, there are significant fluctuations. The middle plot presents the reconstructed top quark mass using the lepton, $b$-jet, and neutrino momenta from \vvflows. Here the separation between the nominal and the two shifted samples is more pronounced. Using the \vvflows model, one can produce an even smoother template by sampling multiple solutions for each event. The right plot shows the reconstructed top quark mass using the lepton, $b$-jet, and neutrino momenta from \vvflows with 256 samples per event, demonstrating the method's potential for improved template construction for mass measurements.}
...
These improvements can increase the sensitivity of the measurement of top quark spin correlation and entanglement at the LHC~\cite{CMS:2015cal}.
