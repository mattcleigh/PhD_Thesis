...
The \Gls{LHC}~\cite{LHCMachine,LHC1,LHC2,LHC3} is a synchrotron collider located at \Gls{CERN} near Geneva, Switzerland.
It is also the world's most powerful particle accelerator, providing both \Gls{pp} and heavy ion collisions at the high-energy frontier.
Protons in the \Gls{LHC} are accelerated until they are only $3~\mps$ slower than the speed of light.
...
Four primary experiments utilize the $\TeV$ scale collisions generated at each of the four \Gls{IPs} of the \Gls{LHC}.
...
The \Gls{LHCb} experiment~\cite{LHCb} focuses on measuring the properties of B mesons and other particles containing b-quarks, which are significant for understanding CP violation.
The \Gls{ALICE} experiment~\cite{ALICE} investigates the properties of quark-gluon plasma, a state of matter believed to have existed in the early universe.
Additionally, the \Gls{LHC} hosts several smaller, highly specialized experiments including LHCf~\cite{LHCf}, TOTEM~\cite{TOTEM}, MoEDAL~\cite{MoEDAL}, FASER~\cite{FASER}, and SND@LHC~\cite{SNDLHC}.
All \Gls{LHC} experiments strive to answer fundamental questions about the universe; to test the predictions of the Standard Model of particle physics and to search for new physics beyond it.

This section describes common collider physics definitions, such as the coordinate system and luminosity,
the \Gls{LHC} along with its injector chain, and the ATLAS experiment.
Discussion is limited to the description of the \Gls{pp} collisions at the \Gls{LHC} as this is the focus of this thesis.

\section{The Large Hadron Collider}

The \Gls{LHC} is a somewhat circular collider with a circumference of $26.7~\km$ housed in a tunnel with a depth varying between $45$~m and $170$~m underground.
...
An additional 392 higher-order multipole magnets are used for beam insertion, cleaning, dumping, and focusing towards the experiment \Gls{IPs}.
...
The \Gls{LHC} was designed to deliver a centre-of-mass energy of $\sqrt{s}=14~\TeV$, but has yet to reach this energy due to technical limitations.
...
The \Gls{LHC} is a synchrotron, meaning that it cannot accelerate particles from rest.
It is only the final stage of the \Gls{CERN} accelerator complex, which is shown in \Cref{fig:cern_accelerator_complex}.
...
Prior to 2020, the chain began with the \Gls{Linac}~2, which accelerated protons to $50~\MeV$.
The input to \Gls{Linac}~2 was hydrogen gas, stripped of its electrons by a strong electric field.
\Gls{Linac}~2 has since been replaced with \Gls{Linac}~4, which accelerates negative hydrogen ions, as opposed to protons.
This change reduces the beam loss, allowing more particles to accumulate in the next stage of the accelerator complex.
Furthermore, \Gls{Linac}~4 introduced a three-fold increase in beam energy, accelerating the particles to $160~\MeV$.
\Gls{Linac}~4 now feeds the \Gls{PSB}, which increases the energy to $2~\GeV$ and also serves to strip the electrons from the hydrogen ions.
From the \Gls{PSB}, the protons are passed to the \Gls{PS}, which accelerates them to $26~\GeV$, and then to the \Gls{SPS}, which accelerates them to $450~\GeV$.
From there, they are injected into the \Gls{LHC}.
This entire process only takes a few seconds, but several minutes are required to completely fill both beams in the \Gls{LHC}.
The \Gls{LHC} then accelerates the beams to their final energy, which, as of 2024, is $6.8~\TeV$ per beam.
...
In collider experiments, the typical coordinate system places the origin at the nominal IP, which, for hermetic detectors like \Gls{ATLAS}, \Gls{ALICE}, and CMS, is at the centre of the detector.
The coordinate systems used by these experiments are right-handed, as shown in \Cref{fig:atlas_coordinate_system}, with the $z$-axis points along the beamline, the $y$-axis pointing upwards, and the $x$-axis pointing towards the centre of the \Gls{LHC} ring.
...
For the \Gls{pp} collisions at the \Gls{LHC}, $S(\theta_c)$ is measured to be approximately $0.61$.
Even with this finite angle, long-range beam-beam interactions can still occur and must be accounted for.

The \Gls{LHC} was designed to deliver $\mathcal{L}(t) = 10^{34}~\unit{\centi\meter^{-2}\second^{-1}}$ at the \Gls{IPs} of ATLAS and CMS.
...
The total integrated luminosity delivered by the \Gls{LHC} and recorded by ATLAS is shown in \Cref{fig:luminosity}.
The \Gls{LHC}'s instantaneous luminosity has increased with each run and this is particularly beneficial for measurements on rare events where the uncertainty is statistically limited.
...
\begin{figure}
    ...
    \caption{The total integrated luminosity delivered by the \Gls{LHC} and recorded by the ATLAS experiment for Run 1 (left)~\cite{run1data}, Run 2 (middle)~\cite{run2data}, and Run 3 (right)~\cite{run3data}.}
    \label{fig:luminosity}
\end{figure}

\subsection{LHC Runs}

The \Gls{LHC} operates on multi-year runs -- the first began in 2010.
...
\begin{table}[h!]
    ...
    \resizebox{\textwidth}{!}{
        \begin{tabular}{lcccccc}
            ...
            Run 2 (2015-2018) & 13                & 156                             & $1.25 \times 10^{11}$ & 25                    & 34     \\
...
As illustrated in \Cref{fig:pileup}, the average number of interactions per crossing has increased with each \Gls{LHC} run.
Typically, only one of the \Gls{pp} interactions, known as the hard scatter, is of primary interest.
...
Filtering out signals from all pileup sources is crucial for accurate data analysis and poses a significant challenge as the \Gls{LHC}'s luminosity increases.
...
The ATLAS experiment~\cite{ATLAS, ATLASRun3} is one of the four primary experiments at the \Gls{LHC}\@.
It is the largest particle detector of the four, weighing $7000$ tonnes and measuring $44$ m long and $25$ m in diameter.
Equipped with multiple sub-detectors, it is a general-purpose detector designed to capture various outgoing products from the \Gls{pp} collisions and explore a wide range of physics phenomena.
...
The \Gls{ID} is the innermost sub-detector of ATLAS and is shown in \Cref{fig:atlas_inner_detector}.
It comprises three sub-detectors: the Pixel Detector, the \Gls{SCT}, and the \Gls{TRT}.
...
The \Gls{ID} is immersed in a $2~\tesla$ axial magnetic field generated by a 5.8 m long superconducting solenoid magnet containing over $9\km$ of NbTi wire.
...
The relative $\pt$ resolution of the \Gls{ID} is described as,
\begin{equation}
...
The high-granularity silicon pixel detector~\cite{ATLASPixel} covers the innermost region of the \Gls{ID}.
...
Between Run 1 and 2, an additional layer was added at a radius of $33~\mm$ to the beamline, known as the \Gls{IBL}~\cite{ATLASIBL}.
Pixels in the \Gls{IBL} measure $50~\um$ in the transverse direction and $250~\um$ in the longitudinal direction.
...
The \Gls{SCT}~\cite{ATLASSCT} surrounds the Pixel Detector, occupying the region between $299~\mm$ and $514~\mm$ from the beamline.
It comprises four concentric barrel layers with nine disks at each end-cap covering the region $|\eta| < 2.5$.
Like the pixel detector, the \Gls{SCT} is composed of silicon readout channels. Unlike the pixels, each microstrip measures $20~\um \times 12~\cm$ and thus only has high granularity in a single direction, oriented to the transverse momentum.
To improve longitudinal resolution, each \Gls{SCT} layer includes two back-to-back microstrip layers, rotated by $40 \unit{\milli\radian}$ relative to each other.
The \Gls{SCT} in total has around 6.3M readout channels.

\subsubsection{TRT}

The outermost sub-detector of the \Gls{ID} is the \Gls{TRT}~\cite{ATLASTRT}.
...
The \Gls{TRT} provides the most hits per track of the three sub-detectors -- approximately 35 hits on average.
...
The \Gls{TRT} comprises approximately 300k thin-walled drift tubes, each $4~\mm$ in diameter.
...
The \Gls{ECal} is specialized for measuring the energy of electrons and photons.
Surrounding this is the \Gls{HCal}, which is designed to measure the energies of hadrons.
...
The \Gls{ECal}~\cite{ATLASECal} is a high granularity calorimeter which uses liquid argon as the active material and lead as the absorber.
To keep the argon in a liquid state, the \Gls{ECal} is cooled to $-185~\unit{\degree C}$.
...
The barrel \Gls{ECal} covers the range $|\eta| < 1.475$ and is split into three layers, decreasing in granularity with distance from the beamline.
The inner layer allows for more precise shower measurements which assists in the identification of pions.
The \Gls{EMEC} and covers the range $1.375 < |\eta| < 3.2$.
Unlike the \Gls{ID}, the \Gls{ECal} resolution increases with incident particle energy.

\subsubsection{Hadronic Calorimeter}

The \Gls{HCal} (or Tile Calorimeter)~\cite{ATLASHCal} surrounds the \Gls{ECal} and is engineered to accurately measure the energy of hadrons.
It uses steel as the absorber and scintillating tiles as the active material made from polystyrene.
The hadrons are contained by losing energy through inelastic nuclear interactions with the steel, producing a shower of radiation which is detected by the tiles.
In the forward regions of the calorimeter, $1.5 < |\eta| < 3.2$, a copper/LAr combination is used.
The granularity and energy resolution of the \Gls{HCal} are lower than the \Gls{ECal}.
...
Though they leave tracks in the \Gls{ID}, the \Gls{MS}~\cite{ATLASMuon,ATLASRun3} provides triggering information, identification, and extra momentum measurements for muons.
The \Gls{MS} is the outermost sub-detector of ATLAS and is shown in \Cref{fig:atlas_muon_spectrometer}.
...
Monitored Drift Tubes are the spectrometer's primary tracking component and are similar in principle to the straw tubes in the \Gls{TRT}\@.
...
Three large air-core toroidal magnets generate the magnetic field in the \Gls{MS}\@, one in the barrel region and one in each end-cap regions.
...
Track reconstruction involves collecting hits registered by the \Gls{ID} and organizing them into distinct tracks, which are most likely produced by individual charged particles.
...
A single particle may deposit energy in multiple adjacent pixels or strips in the \Gls{ID}\@.
...
These hits are first combined into clusters which are then used to form \Gls{SP} defined in three dimensions.
For the \Gls{SCT}, these clusters also combine information from the two back-to-back layers.
...
Tracks are \textit{seeded} by grouping triplets of SPs in three sequential layers in either the pixel or \Gls{SCT} subdetectors.
...
The only requirement to improve purity is that one additional \Gls{SP} must be found outside the seeding region which compatible with the candidate.
...
The score takes into account several factors; the number of \Gls{SP} assigned to the track will increase its score, while the number of holes will decrease it.
Holes are defined as intersections between the reconstructed track and active sensor material where no matching \Gls{SP} is found.
...
Finally, information from the \Gls{TRT} is used to extend the tracks.
A similar Kalman filter incorporates \Gls{TRT} hits starting from the predicted intersection between each silicon track and the \Gls{TRT}.
If successful, the \Gls{TRT} hits are added to the track, and the whole track is fitted once more using a global $\chi^2$ fitter.
The inclusion of \Gls{TRT} hits enhances momentum resolution and allows for particle identification.
However, the extension may be rejected if it worsens the $\chi^2$ of the track candidate or if more than 50\% of the \Gls{TRT} hits are tube hits with no leading edge.
...
Primary vertices are those produced by the many \Gls{pp} collisions during the bunch crossing.
...
It is sometimes ambiguously referred to as the \Gls{PV} while the others are referred to as \textit{pileup} vertices.

Secondary vertices are those produced by the decay of particles originating from the \Gls{PV}.
...
From a detector perspective, jets result in large localized energy deposits in both calorimeters and many tightly packed tracks in a cone emanating from the \Gls{PV}.
...
The general approach for Run 2 and 3 data is to include both sets of information using the \Gls{PFlow} algorithm~\cite{PFlow}.

\subsubsection{Particle Flow}

The \Gls{PFlow} algorithm is designed to prevent the double-counting of momentum contributions from charged particles that leave both \Gls{ID} tracks and calorimeter energy deposits.
This algorithm iterates through the tracks in descending \pt order, constructing charged \Gls{PFlow} objects.
...
These, and the remaining clusters after the subtraction step, are used to construct neutral \Gls{PFlow} objects.

The output of the \Gls{PFlow} algorithm is a collection of charged and neutral \Gls{PFlow} objects, each with defined energy and momentum.
Charged \Gls{PFlow} objects not matched to the \Gls{PV} are removed to reduce pileup contamination.
...
This track association is beneficial for the ATLAS experiment because it enables using the \Gls{JVT}~\cite{JVT}.
The \Gls{JVT} leverages the transverse momenta from associated tracks to discriminate against jets originating from pileup interactions.
Furthermore, many of the flavour tagging algorithms mentioned in the next section and in \Cref{sec:flavour_tagging} operate on the set of tracks associated with a jet, even though the jets themselves were clustered from \Gls{PFlow} objects.
...
Analyses at ATLAS make frequent use of the \Gls{D2obs} observable~\cite{ATLASD2} to separate 1-prong and 2-prong jets,
\begin{align}
    \Dtwo(\beta) & = \frac{e_3(\beta)}{e_2(\beta)^3}.
\end{align}
Additionally, the \Gls{EFPs}~\cite{EFP} are a set of basis features which are sensitive to the underlying substructure of different jet types.
...
This distance can be resolved by the \Gls{ID} vertex fitting algorithms and would result in a vertex with a notable transverse displacement, as shown in \Cref{fig:btagging}.
...
A second track-based tagger known as \Gls{DIPS}~\cite{DIPS} was developed in 2020.
It used the deep sets architecture~\cite{DeepSets}, and the corresponding combined tagger was labelled \Gls{DL1d}~\cite{AlexThesis}.
