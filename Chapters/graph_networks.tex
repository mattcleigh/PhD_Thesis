...
The philosophy behind \Glspl{GNN} is the idea that a system is best understood when broken down into compositional elements.
...
Graphs are a way to represent explicitly structured data, encompassing entities and their pairwise relationships.
\Glspl{GNN} are a type of deep learning model designed to handle this structure, allowing information to flow through the network in a manner that mirrors the graph's organization.

This chapter explores the foundational principles of \Glspl{GNN}, how they operate, and their limitations.
It also delves into the most successful \Gls{GNN} variant to date, the transformer model, which is utilized extensively in the subsequent chapters of this thesis.
...
\Glspl{GNN} employ relational reasoning to complete tasks by directly manipulating the rules, entities, and relations of a system.
Rules may be reused across different entities within the system, allowing for more efficient learning and greater generalization.

An example of successful relational reasoning being used in deep learning outside \Glspl{GNN} is in image processing tasks.
An image is typically represented as a rank-3 tensor using the pixel values as the tensor elements.
This tensor has shape $(H, W, C)$, where $H$ and $W$ describe the image resolution, and $C$ is the number color channels.
If one wanted to process this tensor with an \Gls{MLP}, it must first be flattened to a have of shape $H \times W \times C$.
...
Alternatively, a network composed of learnable convolutional kernels can be used.
...
This reduces the number of parameters required in the model.

\section{Defining a Graph}
\label{sec:graph_definition}

A graph $\mathcal{G}$ is a data structure that at its most basic level consists of a set of $N$ attributed nodes $\mathcal{N} = \{\x_i\}_{i=1}^{N}$.
Here, $\x_i$ is the attribute of the $i$-th node.
Two nodes may be connected by an edge.
Each edge possess its own attributes and has a defined direction, designating one node as the sender and the other as the receiver.
Thus, the edge is defined by a tuple $(\edge_k, r_k, s_k)$, where $\edge_k$ is the attribute of the edge, and $r_k$ and $s_k$ are the indices of the receiver and sender nodes $\mathcal{N}$ respectively.
All edges in the graph are defined by the set of $N^e$ attributed edges $\mathcal{E} = \{(\edge_k, r_k, s_k)\}_{k=1}^{N^e}$.
...
The structure of the graph is only defined by the interconnections between the nodes.
The actual ordering of the nodes in the set is arbitrary, and any operation on the graph should not depend on this order.
...
Multiple sets of edges are allowed to exist between nodes $\{\mathcal{E}^1, \mathcal{E}^2, \ldots\}$, defining a multi-graph.
Each of the edge $\edge_i$, node $\x_i$, and global $\u$ attributes can also be encoded as any type of representation, even graphs themselves.
In most use cases dealt with in this thesis, each of $\x_i$, $\edge_k$, and $\u$ are real valued tensors.
It is also common for all the nodes and edges to share same dimensionality, imposing an equivalence between the elements of the set.
This simplifies the design of models operating on the graph as it allows operations on the individual elements to be parameterized with standard building blocks, such as affine layers, \Glspl{MLP}, and \Glspl{CNN}.
...
Almost all \Glspl{GNN} operate by facilitating message passing between the nodes of the graph.
...
In this example, it might be more natural to represent the particles in the Feynman diagram as edges rather than the nodes.
...
Though graphs typically have no natural ordering, text and image patches would not make sense if shuffled.
Therefore, order has to be imposed by the model.
...
When constructing a graph representation it is crucial to consider its interconnectivity, i.e. which pairs of nodes have edges between them.
\Glspl{GNN} propagate information along edges, so the structure of the graph can have a significant impact on the model's performance.
Interconnectivity is closely tied to two key issues in \Glspl{GNN}: over-squashing~\cite{OverSquashing} and the over-smoothing~\cite{OverSmoothing}.
...
Secondly, is that it exacerbates the other issue with \Glspl{GNN}, over-smoothing.
...
This one of the reasons that the depth of most \Glspl{GNN} is notably shallower than other deep learning models, as with every round of message passing each node aggregates information from its neighbours, diluting the original information.
...
As previously mentioned, one of the key inductive biases leading to the success of \Glspl{CNN} is the locality of the convolutional kernel.
However, the state-of-the-art models in computer vision are \Glspl{GNN} operating on fully connected graphs of image patches~\cite{VisionTransformer}, demonstrating that locality in images is not always useful.
...
The \Glspl{GNN} that work well in these cases are equipped with all the tools needed to combat over-smoothing mentioned in the previous section, as well as the attention mechanism~\cite{Attention}.
Attention refers to the learnable process of weighting the incoming messages before aggregation.
It allows the \Gls{GNN} to ``pay more attention'' to specific connections within the graph.

The fact that the state-of-the-art models in computer vision disregard the locality inductive bias raises the point that perhaps we might actually not know what is best for the model.
These models, as shown in \Cref{fig:benchmarks}, however require much more data and computational resources than \Glspl{CNN}.
This raises the point that with enough data, a large enough model, and sufficient training time, enforcing what relationships are important may not be necessary, and it is better to let the model learn these rules itself.
It also raises whether the message passing graph and the input data graph should be tied together at all.
Models such as the perceiver~\cite{Perceiver} project input information onto a completely different latent graph for message passing.
Recent work for \Glspl{GNN} in computer vision has shown that they require extra, fully learnable nodes not tied to any image patch~\cite{VisionTransformersNeed}.
...
The \Gls{GNBlock}~\cite{RelationalInductiveBiases} is a useful building block for constructing deep learning models that operate on graph-structured data.
It offers a framework that generalizes to many other existing graph-based models, such as \Glspl{GCN}~\cite{GCN}, \Glspl{GAT}~\cite{GraphAttentionNetworks}, deep sets~\cite{DeepSets}, and even transformers~\cite{Attention}.
A \Gls{GNBlock} is defined as a series of operations that update the node, edge, and global attributes via the exchange and aggregation of information according to the structure of the graph.
The \Gls{GNBlock} does not change the cardinality of the features present, it only updates their attributes,
\begin{equation}
...
Given an input graph $\mathcal{G} = (\mathcal{N}, \mathcal{E}, \u)$ the \Gls{GNBlock} proceeds as follows.
...
\begin{enumerate}
    \item \textbf{Edge Block:} The edges are updated using the current edge attributes, the attributes of the sender and receiver nodes, and the global attribute.
          ...
          \end{equation}
          All edges throughout the graph are updated in this manner and this step can be executed in parallel.
...
The \Gls{GNBlock} describes a basic graph to graph transformation which can be composed to form a deeper model.
...
For example, the final \Gls{GNBlock} in the stack may only contain an edge block for an edge-level task.

It is important to investigate exactly what inductive biases are being imposed by the \Gls{GNBlock}.
...
The \Gls{GNBlock} applies the same update and aggregation functions to all nodes and edges in the graph.
...
Furthermore, as the \Gls{GNBlock} only prescribes the rules for how the features of a graph are updated; it is agnostic to the structure and size of the input graph.
...
Almost all \Glspl{GNN} variants facilitate message passing can be thought of as some specialization of the \Gls{GNBlock} depending on the forms of the $\phi$ and $\rho$ functions and the existence of edge, node, and global features.
A full \Gls{GNBlock} is shown in \Cref{fig:full_gn_block}.
A basic implementation, where all graph attributes are tensors, can be done using \Glspl{MLP} for each of the update functions, and summation for the aggregation functions.
This creates the update shown in \Cref{alg:gn_block}, where the square brackets denote tensor concatenation.
Even the deep set, which has no pairwise interactions between the nodes can be seen as a reduced form of a \Gls{GNBlock}, as shown in \Cref{fig:deep_set}.
...
\begin{algorithm}
    \caption{Basic \Gls{GNBlock} using \Glspl{MLP}}
    \label{alg:gn_block}
    \begin{algorithmic}[1]
...
\begin{figure}
    ...
    \begin{subfigure}[b]{0.49\textwidth}
        ...
        \label{fig:deep_set}
    \end{subfigure}
    \caption{Examples of different \Gls{GNBlock} implementations \subref{fig:full_gn_block} A full \Gls{GNBlock} has persistent edge and global features. \subref{fig:deep_set} A deep set only applies pooling to the node features to produce a global feature. Adapted from \textcite{RelationalInductiveBiases}.}
\end{figure}
...
Originally designed for \Gls{NLP} sequence-to-sequence tasks, transformers have since become the dominant model in many areas of deep learning.
They marked such an improvement that research in other sequence-based models, such as recursive neural networks, almost ceased entirely.
They also marked a massive leap in commercializing deep learning for \Gls{NLP}.
...
In 2020, transformers were adapted to work on image data~\cite{VisionTransformer}, marking the point where transformers took the crown in computer vision benchmarks from \Glspl{CNN} as shown in \Cref{fig:benchmarks}.
...
These models do not natively run on sequences, but on sets, and must actually be adapted to work on data with an inherent order.
Two main types of transformers are described, the transformer encoder and the transformer decoder.
The \Gls{TE} is a variant of the \Gls{GNBlock} called a \Gls{NLNN}~\cite{NonlocalNeuralNetworks} as shown in \Cref{fig:transformer}.
This function operates on a pure set, with no persistent edge or global features.
It involves and message passing step equipped with an attention mechanism, followed by a node update step using an \Gls{MLP}\@.

The jargon of transformers differs slightly from that of \Glspl{GNN}.
...
\begin{figure}
    ...
    \caption{\Gls{NLNN} / Transformer Encoder}
...
There are many of ways to parameterize the message passing step in a \Gls{GNN}, and the one used in the transformer is the \Gls{SDPA}.
In \Gls{SDPA}, the message passed between nodes is a linear projection of the sender node attributes.
...
The message weight is based on a dot product between additional linear projections of the sender node, called they \textit{key} $\k_{i} = \mathbf{W}^k \x_i$, and the receiver node, called the \textit{query} $\q_{j} = \mathbf{W}^q \x_j$.
Aggregation is simply a weighted mean of the incoming messages, where a softmax operation is applied over the dot products to ensure that they sum to one.
...
In the framing of the \Gls{GNBlock}, \Gls{SDPA} factorizes the edge update into a tuple containing a scalar expressing the message weight $\alpha_k'(\x_{s_k}, \x_{r_k})$ and a vector function for calculating the message content $\mathbf{v}_k'(\x_{s_k})$.
...
\begin{equation}
    \begin{aligned}
        ...
        \mathbf{\bar\edge}_i & = \rho^{e \to x}(E_i)                                                                                                        \\
                             & = \frac{\sum_{k: r_k = i} \alpha_k' \mathbf{v}_k'}{\sum_{k: r_k = i} \alpha_k'},
    \end{aligned}
\end{equation}
...
\begin{equation}
    ...
    \end{aligned}
\end{equation}
In this expression, the tensor representing the sender nodes $\X_S \in \mathbb{R}^{N_S \times d}$ is distinguished from the tensor representing the receiver nodes $\X_R \in \mathbb{R}^{N_R \times d}$.
...
Transformers typically extend this operation to \Gls{MHA}, as shown in \Cref{fig:mha}.
This is where \Gls{SDPA} is performed $H$ times in parallel with different matrices $\mathbf{W}^q$, $\mathbf{W}^k$, $\mathbf{W}^v$, and $\mathbf{B}$.
...
\begin{equation}
    ...
    \end{aligned}
\end{equation}
In \Gls{MHA}, many types of messages are passed between the nodes and one can think of this as constructing a multi-graph on the set.
Both self- and cross-attention can be multi-headed where \Gls{MHSA} is defined as,
\begin{equation}
    \text{MHSA}(\X) = \text{MHA}(\X, \X).
\end{equation}
...
The dot product in the \Gls{SDPA} layer necessitates that the dimensions of key and query tensors are equal.
Additionally, the majority of \Gls{SDPA} implementations do not resize, resulting in $\mathbf{Q}$, $\mathbf{K}$, and $\mathbf{V}$ all having the same shape.
When extending to \Gls{MHA}, it is standard to split the dimensionality equally by the number of heads $H$.
This means that a single \Gls{MHA} function is typically parameterized by four square matrices corresponding to $4d^2$ learnable parameters with the number of heads $H$ being the sole hyperparameter of the block.
...
While many of the message passing variants require custom libraries and are challenging to implement from scratch, \Gls{MHA} can be implemented in PyTorch using standard linear algebra operations, as demonstrated in \Cref{code:attention}.
...
While the message passing operation is considerably simpler than using \Glspl{MLP} for each stage as described in \Cref{alg:gn_block}, it is possible to enhance the model's expressivity by scaling either the model dimension or the number of heads.
Self-attention requires $O(N^2)$ many operations, where $N$ is the number of nodes in the set.
Its memory usage however is $O(N)$, as there exist algorithms that perform the update without ever needing to manifest the full attention matrix~\cite{SelfattentionDoesNot, FlashAttentionFastMemoryEfficient}.
...
% \Gls{SDPA} stands out as an exceptionally efficient method for expressive message passing in fully connected graphs.
% To illustrate this, we can start by examining the simplest \Gls{GNN} variant, the GCN~\cite{GCN}, and identify its limitations.
...
% The basic \Gls{GNBlock} in \Cref{alg:gn_block} does this by concatenating the sender and receiver node attributes before passing them through an MLP\@.
...
% The argument made here is that one will struggle to find a more simple and efficient way to parameterize the message passing step in a \Gls{GNN} that does not lead to over-smoothing when fully connected.
...
A single transformer block generally includes two types of sub-blocks: \Gls{MHA} for message passing and \Gls{FFN}/\Gls{MLP} layers for the node update.
These sub-blocks combine and repeat to form the larger transformer block.
\Gls{MHA} layers can be configured as self-attention or cross-attention, and are typically stacked with FFN layers in series~\cite{Attention}, but sometimes in parallel~\cite{Palm}.
...
Most transformer layers use an \Gls{MLP} with a single hidden layer that expands the node attributes' dimensionality by a factor of $M$.
However, modern transformers use \Gls{GLU}~\cite{GLU}, a network layer involving the element-wise multiplication of two linear projections, one of which is activated~\cite{SwiGLU},
\begin{equation}
...
This was found to excessively squash the signal; therefore, contemporary transformers often utilize the \Gls{PreNorm} configuration~\cite{PreLN}.
Extra normalization within the sub-blocks is also common.


\subsubsection{Transformer Encoder}

A single PreNorm \Gls{TE} block is shown in \Cref{fig:transformer_blocks}.
The update is given by,
\begin{equation}
    \begin{aligned}
        \X & \leftarrow \X + \text{MHSA}(L_\text{norm}(\X)), \\
        \X & \leftarrow \X + \text{FFN}(L_\text{norm}(\X)).
    \end{aligned}
\end{equation}
A full \Gls{TE} is composed of many \Gls{TE} Blocks stacked in series forming a permutation equivariant set to set transformation.
For many tasks, the \Gls{TE} is the main feature extractor.

\subsubsection{Transformer Decoder}

To condition the update of one set on another, a \Gls{CA} block, can be utilized.
For a more expressive operation, the layer can include both self-attention and cross-attention, creating a \Gls{TD} block, as shown in \Cref{fig:transformer_blocks}.
The update of a \Gls{TD} block is written as,
\begin{equation}
    \begin{aligned}
    ...
    \end{aligned}
\end{equation}
The \Gls{TE} and \Gls{TD} are typically combined to create a set-to-set transformation that first extracts features from the input set, and the conditions the output set on these features.
...
When viewing the transformer as a \Gls{GNN}, the inclusion of edge information becomes intuitive.
...
In the \Gls{SDPA} operation in \Cref{eq:attention}, the attention matrix defined by,
\begin{equation}
    \mathbf{A} = \text{softmax}\left( \frac{\mathbf{Q} \mathbf{K}^T}{d_k} + \mathbf{B} \right),
\end{equation}
which has the shape $(N, N)$ for self-attention.
Extending this to \Gls{MHA} it gains an extra dimension $(N, N, H)$ aligning with the required shape of the edge tensor and making it the only valid candidate in the entire operation.
...
The \Gls{ParT}~\cite{ParticleTransformerJet} uses a \Gls{TE} to embed a set of particles, defined by their kinematics.
For each pair of particles, specific edge features are constructed using physically motivated interaction terms~\cite{LundJetPlane}.
The model uses an \Gls{MLP} to embed these edge features into a tensor of shape $(N, N, H)$, which becomes the attention bias in each of the \Gls{TE} Blocks.
...
Many sequence generating models are called ``decoder only'', however in the presented framework, these models are \Gls{TE} models with a causal attention mask.

While a causal mask is sufficient for a transformer to grasp order~\cite{TransformerLanguageModels}, it is often complemented by \textit{positional encoding}.
\Gls{APE} integrates sequence positions into node attributes.
...
In images, \Gls{APE}, often termed spatial encoding, involves encoding and adding the 2D coordinates of a patch to its representation~\cite{VisionTransformer}.

\Gls{APE} is limited in capturing relative positions, necessitating the use of \Gls{RPE}~\cite{SelfAttentionRelativePosition}.
\Gls{RPE} biases the attention matrix, serving as edge information, with various implementations.
...
In \Cref{alg:gn_block}, the global attributes are updated by summing the node attributes which is then passed through an \Gls{MLP}\@.
However, a \Gls{CA} block provides a more expressive pooling operation, even when the cardinality of the receiving set is one.
This is known as class-attention~\cite{GoingDeeper}, and is utilized in vision transformers for image classification.
These models typically have several \Gls{TE} layers for feature extraction, followed by a few \Gls{CA} layers to repeatedly pool the features into a single global attribute which is used for classification.
...
This is shown for a \Gls{TE} block in \Cref{fig:adaptive_gating} and can be written for a single sub-block $f$ as,
\begin{equation}
    \begin{aligned}
