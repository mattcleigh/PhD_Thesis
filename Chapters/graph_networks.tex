...
GNNs are a type of deep learning model designed to handle this structure, allowing information to flow through the network in a manner that mirrors the graph's organization.
...
This simplifies the design of models operating on the graph as it allows operations on the individual elements to be parameterized with standard building blocks, such as affine layers, MLPs, and CNNs.
...
These models, as shown in \Cref{fig:benchmarks}, however require much more data and computational resources than CNNs.
...
Recent work for GNNs in computer vision has shown that they require extra, fully learnable nodes not tied to any image patch~\cite{VisionTransformersNeed}.
...
A GN Block is defined as a series of operations that update the node, edge, and global attributes via the exchange and aggregation of information according to the structure of the graph.
The GN Block does not change the cardinality of the features present, it only updates their attributes,
\begin{equation}
...
Originally designed for NLP sequence-to-sequence tasks, transformers have since become the dominant model in many areas of deep learning.
...
In the framing of the GN Block, SDPA factorizes the edge update into a tuple containing a scalar expressing the message weight $\alpha_k'(\x_{s_k}, \x_{r_k})$ and a vector function for calculating the message content $\mathbf{v}_k'(\x_{s_k})$.
...
Both self- and cross-attention can be multi-headed where multi-headed self-attention (MHSA) is defined as,
\begin{equation}
