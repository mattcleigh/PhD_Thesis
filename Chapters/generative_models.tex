...
\Glspl{PGM} are a class of models that either aim to learn the underlying probability distribution of a dataset or at least facilitate means to generate new samples indistinguishable from the data.
It is worth highlighting that there is conflicting terminology and many overlapping definitions in the field of \Glspl{PGM}~\cite{DiscriminativeVsGenerative, MachineLearningDiscriminative}, so terms in this thesis will be explicitly defined.

Given a training set of \Gls{iid} samples $\mathcal{D} = \{\x_i\}_{i=1}^N$, it is assumed that the data is generated from some unknown probability density $p_\X(\x): \mathcal{X} \rightarrow \mathbb{R}$ over a random variable $\X \in \mathcal{X}$.
A \Gls{PGM} with parameters $\theta$ can be fit using $\mathcal{D}$, and would allow for the generation of new samples following a close approximation of $p_\X(\x)$, denoted as $p_{\theta}(\x)$.

There exist many types of \Glspl{PGM}; some models such as Normalizing Flows (NFs)~\cite{VariationalInferenceNormalizing} provide an explicit functional form for the density $p_{\theta}(\x)$, while others like \Glspl{GAN}~\cite{GenerativeAdversarialNetworks} implicitly learn the density but only permit sampling from it.

It is often more interesting to consider the conditional distribution $p_{\X}(\x|\con)$, where $\con$ is some context variable.
A conditional \Gls{PGM} can be used to generate samples from the conditional distribution, $p_{\theta}(\x|\con)$.
...
This section covers the theory behind the \Glspl{PGM} used elsewhere in this thesis, namely the NFs and diffusion models used in \Cref{ch:neutrino_unfolding,ch:jet_generation,ch:foundation_models}.
...
At the start of this thesis, applications \Glspl{PGM} in HEP mainly involved \Glspl{VAE}~\cite{AutoEncodingVariationalBayes} and \Glspl{GAN}~\cite{GenerativeAdversarialNetworks}.
...
While it is remarkable how quickly the focus on \Glspl{VAE} and \Glspl{GAN} has moved on, it is still important to highlight them here for historical context and to better understand the strengths of the models that have replaced them.

\subsection{Variational Autoencoders}

\Glspl{VAE} were introduced in 2013~\cite{AutoEncodingVariationalBayes} and quickly became a popular model for generative tasks, unsupervised learning, and variational inference.
...
\Glspl{VAE} introduce an additional parametrized distribution to approximate the posterior which is also taken to be Gaussian with a diagonal covariance matrix,
\begin{equation}
...
\Glspl{VAE} are simple to train but have several limitations.
...
\Glspl{VAE} were never particularly good at generating high-quality samples, but attempts were still made in HEP to use them for fast simulation of detector responses~\cite{VariationalAutoencodersJet, GraphVariationalAutoencoder, ParticlebasedFastJet}.
...
Until recently, \Glspl{GAN} were the state-of-the-art generative models for producing high-quality samples, most notably demonstrated in generating high-resolution images~\cite{StyleGAN2, StyleGAN3}.
\Glspl{GAN} are based on a two-player min-max game between a generator $G_\theta(\z)$ and a discriminator $D_\phi(\x)$, each a neural network with parameters $\theta$ and $\phi$ respectively.
...
The most widely used is the original non-saturating \Gls{GAN}~\cite{GenerativeAdversarialNetworks}.
...
\end{align}
Other variants that use different loss derivations exist, such as the Wasserstein \Gls{GAN}~\cite{WGAN1} and the Geometric \Gls{GAN}~\cite{GeometricGAN}.
...
The main drawback of \Glspl{GAN} is that they are challenging to train.
...
Another drawback to \Glspl{GAN} is that it is challenging to produce a conditional generator $G_\theta(\x|\con)$.
...
In HEP, \Glspl{GAN} have mainly been trialled as a method for fast simulation~\cite{MPGAN, GAPT, CaloGAN, EPICGAN}.
However, the main issue is that \Glspl{GAN} tend not to cover the full data distribution.
Furthermore, fast simulation requires conditional generation, as detector response depends on the properties of the incoming particle, making \Glspl{GAN} not optimal for this task.
...
Like \Glspl{GAN} and \Glspl{VAE}, they prescribe a method to train a transformation from a simple latent distribution into one that matches the data.
However, unlike \Glspl{GAN} and \Glspl{VAE}, NFs allow for explicit likelihood evaluation, making them much easier to train and well-suited for density estimation.
...
Whereas in a standard NF, the transformation is composed of a fixed number of sub-transformations or flow layers, a \Gls{CNF}~\cite{NeuralODE} defines a continuous process where each infinitesimal transformation depends on the current state of the data and the time index.
This leads to an \Gls{ODE} of variable $\x_t \in \mathbb{R}^d$ indexed by $t \in [0, 1]$ defined by a smooth time-varying vector field $u(\x_t, t)$\footnote{The notation $u_t(\x_t)$ and $u(\x_t, t)$ are used interchangeably.},
\begin{equation}
    \label{eq:cnf}
    \diff \x_t = u_t(\x_t) \diff t.
\end{equation}
Given $\x_0 \sim p_0$, the vector field creates a pushforward $p_t = [\phi_t]_\# p_0$, where $\phi_t$ is the solution of the \Gls{ODE},
\begin{equation}
...
\end{equation}
The velocity field $u_t$ is said to be the \textit{probability flow} \Gls{ODE}~\cite{ScoreBasedGenerativeModeling} and $p_t$ is the \textit{probability path} generated by $u_t$.
...
Like any flow, the \Gls{CNF} can be trained by maximizing the log-likelihood of the data,
\begin{equation}
    \label{eq:cnf_loss}
    \mathcal{L}(\theta) = \mathbb{E}_{\x \sim \mathcal{D}}\left[\log p_0(\x) - \int_0^1 (\nabla \cdot u_\theta(\x_t, t)) \diff t\right].
\end{equation}
Once the transformation is defined, any numerical \Gls{ODE} solver can transform samples from the data space to the latent space and back again.
This introduces an extra level of complexity when evaluating the model.
A single trained model can have a wide range of behaviour depending on the choice of \Gls{ODE} solver, the number of integration steps, or the use of adaptive step sizes.
...
\Glspl{CNF} are highly expressive models, but are slow to train.
...
Even with these optimizations, a single training iteration still requires a full numerical integration of the \Gls{ODE} and subsequent backpropagation through each step, making \Glspl{CNF} impractical for high-dimensional data.


\section{Diffusion Models}

Over the past four years, diffusion models have emerged as the standard for generating high-dimensional data.
Their impact on the landscape of \Glspl{PGM} has been profound.
...
This significant advancement is primarily due to the exceptionally straightforward training of diffusion models, especially compared to \Glspl{CNF} and \Glspl{GAN}.
...
Finally, the quality of the samples drawn from a diffusion model outperforms all other \Glspl{PGM}.
...
The true research frenzy began in 2021 when diffusion models officially dethroned \Glspl{GAN} as the state-of-the-art for conditional image generation~\cite{DiffusionBeatsGANS}.

Many competing, overlapping, and sometimes equivalent frameworks are used to define diffusion models.
They are described through the lens of Markov chains trained via variational inference~\cite{DDPM, DDIM}, score-based models with Langevin Dynamics~\cite{ScoreBasedGenerativeModeling, ElucidatingDesignSpace}, and flow matching with \Glspl{CNF}~\cite{BuildingNormalizingFlows, FlowMatchingGenerative, FlowStraightFast}.
...
The forward-process involves progressively applying Gaussian perturbations to a data sample in the continuous such that it can be modelled as an \Glspl{SDE},
\begin{equation}
    \label{eq:sde}
...
At $t=0$, the sample is uncorrupted\footnote{Note that the time index in diffusion models is typically inverted compared to \Glspl{CNF}}.
...
Remarkably, the \Glspl{SDE} in \Cref{eq:sde} has an exact inverse~\cite{ReversetimeDiffusionEquation} which can be used to define the reverse-process,
\begin{equation}
...
The only unknown term in \Cref{eq:reverse_sde}, $\score$, is referred to as the score function and thus approximating it, \Gls{Sm}, is the main objective of a score-based generative model.

One naive approach to perform \Gls{Sm} is to use direct regression via mean-squared-error which results in the loss function,
\begin{equation}
    \label{eq:naive_loss}
    \mathcal{L}_{Sm}(\theta) =
...
One can instead use the \Gls{DSm} objective~\cite{ScoreMatching, SlicedScoreMatching} where the conditional score function $\cscore$ is approximated,
\begin{equation}
...
Once the model is trained, one can define the sampling procedure, which starts from samples drawn from $p_Z=\mathcal{N}(0, \sigma(T) \mathbb{I})$ and numerically integrates the reverse \Glspl{SDE} in \Cref{eq:reverse_sde} from $t: T \rightarrow 0$ to obtain samples matching $p_0$.
...
Like \Glspl{CNF}, the generation quality is highly sensitive to the number of integration steps as each step requires another forward pass through the neural network.
The high computational cost of generating a sample with a diffusion model is the main drawback of this approach compared to single-pass methods like NFs and \Glspl{GAN}.

As a method to speed up generation, \textcite{ScoreBasedGenerativeModeling} proved the existence of the probability flow \Gls{ODE} whose solutions share the same marginal densities $p_t(\x_t)$ as the \Glspl{SDE}.
For the \Glspl{SDE} defined in \Cref{eq:sde} it is given by,
\begin{equation}
    \label{eq:probability_flow}
    \diff \x_t = \left[f(\x_t, t) - \frac{1}{2} g(t)^2 \score\right] \diff t.
\end{equation}
Empirically, the probability flow \Gls{ODE} provides a larger tolerance to truncation errors, allowing for fewer integration steps and, thus, faster generation.
In addition, it enables the use of more advanced \Gls{ODE} solvers, provides deterministic encoding, and allows for log-likelihood computation via \Cref{eq:cnf_loglikelihood}, indicating the strong connection between score-based models and \Glspl{CNF}.
Furthermore, while the probability flow \Gls{ODE} was initially introduced as an alternative method for generation, \textcite{ElucidatingDesignSpace} showed that the entire training objective of a score-based model can be derived purely from an \Gls{ODE} perspective.

\subsection{Conditional Flow Matching}
\label{sec:cfm}

\Gls{CFM}~\cite{BuildingNormalizingFlows, FlowMatchingGenerative, FlowStraightFast} is an alternative framework which arrives at an equivalent training objective to score-based models and can be seen as a method to construct \Glspl{CNF} without the need for numerical integration during training.
...
However, they approach the problem without using \Glspl{SDE}, and like \textcite{ElucidatingDesignSpace}, directly target the probability flow \Gls{ODE}, arguably yielding a more straightforward derivation.

Like \Glspl{CNF}, the goal of \Gls{CFM} is to learn the velocity field $u_t(\x_t)$ that induces the time-dependent density $p_t(\x_t)$ adhering to the boundary conditions $p_1 = p_\X$ and $p_0 = p_\Z$\footnote{Note again that the time index is inverted compared to score-based models.}.
...
Once the network is trained, one can sample from the model by integrating the conditional vector field from noise sample to the data sample as with any \Gls{CNF}.
...
A trained network defines the \Gls{ODE} or \Glspl{SDE}, which can be used to generate samples via numerical integration.
Specifics in this process depend on the choice of framework, the network architecture, and the hyperparameters used.

Over the past four years, some best practices have emerged for training and sampling from diffusion models, such as using slow moving offline networks, targeted time sampling, time encoding, and more advanced \Gls{ODE} solvers.
...
The first uses the \Gls{Sm} objective by~\textcite{GenerativeModelingEstimating} with trigonometric interpolations~\cite{StochasticInterpolants, ImprovedDenoisingDiffusion}, which is referred to as \Gls{Sm}-TI.
The second follows the work by~\textcite{ElucidatingDesignSpace} often referred to as \Gls{EDM} diffusion, named so after the title of the paper in which the method was introduced, ``Elucidating the Design Space of Diffusion-Based Generative Models''.
The third is the \Gls{CFM} framework by~\textcite{FlowMatchingGenerative, FlowStraightFast, BuildingNormalizingFlows}.
...
In \Gls{Sm}-TI, the diffusion schedule is chosen to be a cosine and sine function such that the total power of $\x_t$, assuming that the training set was standardized, is constant across the schedule.
...
This is then used to approximate the score function, which is used in either the reverse \Glspl{SDE} or the probability flow \Gls{ODE} to generate samples.

In \Gls{EDM}, the diffusion schedule is constructed to simplify the form of the probability flow \Gls{ODE} as much as possible.
Noise is added at a linearly increasing rate to a signal that is kept constant, $\x_t = \x_0 + t\z$.
This results in an \Gls{ODE} with straighter trajectories, which is beneficial for the numerical integration during sampling as it reduces the truncation errors.
...
One can take steps towards the estimated data sample during integration with any \Gls{ODE}.
While the training objective in \Gls{EDM} diffusion was derived without the need for an \Glspl{SDE}, the authors also showed that using a stochastic sampler can improve the quality of the generated samples if enough integration steps are taken.

The \Gls{CFM} framework follows the derivation in \Cref{sec:cfm}, where the diffusion schedule is a basic linear interpolation between the noise and the data and the network target is simply the difference between the two.
\Gls{CFM} has arguably the simplest to implement training loop, which is shown in \Cref{code:cfm}.
This model generated the samples in \Cref{fig:cm_samples}, which includes double-moon and four-box datasets.
One of the main benefits of the \Gls{CNF} framework is that the latent distribution does not have to be Gaussian, and the theory holds for any arbitrary distributions.
...
\begin{figure}
    ...
    \end{minted}
    \caption{A simple implementation of the \Gls{CFM} framework in PyTorch. The model is trained to predict the velocity field $u_t(\x_t)$ that interpolates between noise $\x_1$ and data $\x_0$ at time $t$.}
    \label{code:cfm}
\end{figure}
...
However, the \Gls{CFM} framework with the linear interpolation diffusion schedule is now used by the majority of commercial diffusion models for text-to-image synthesis.
        ...
        \begin{table}[h!]
            ...
            \resizebox{\textwidth}{!}{
                \begin{tabular}{cccc}
                    ...
                    sampling                     & \Glspl{SDE} or \Gls{ODE}                              & \Glspl{SDE} or \Gls{ODE}                   & \Gls{ODE}              \\
                    \bottomrule
                \end{tabular}
            }
        \end{table}
...
\begin{figure}[ht]
    ...
    \caption{The transformation of Gaussian samples into a double-moon distribution (top) and a four-box distribution (middle), and the interpolation between the two (bottom). The model was trained using the \Gls{CNF} framework generated with Heun's method~\cite{heun} for 64 steps. Each column shows the samples at a different time index $t$, the black lines trace back to the samples in the previous column. The rightmost column shows the full paths traced by each sample from the base distribution.}
...
One of the claims of both \Gls{EDM} and \Gls{CFM} is that they lead to straighter trajectories than \Gls{Sm}-TI, allowing for faster sampling as fewer steps are needed in the numerical integration.
This is indeed the case, as shown by \Cref{fig:vp_vs_cfm}, where the \Gls{Sm}-TI model results in paths with very high curvature, especially at the end of the schedule.
...
So, even if the conditional paths are straight, as with \Gls{EDM} and \Gls{CFM}, the marginal paths are not.
...
\begin{figure}[ht]
    ...
    \caption{The marginal paths generated interpolating between a Gaussian and a double-moon distribution learned by using the \Gls{CFM} schedule (left) and the \Gls{Sm}-TI schedule (right).}
    \label{fig:vp_vs_cfm}
\end{figure}

Using conditional paths as a proxy training target results in unbiased updates with high variance.
While diffusion models are not at risk of collapsing like \Glspl{GAN}, gradient updates are highly stochastic, and convergence is slow.
Almost all diffusion models are trained with two copies of the network: an online-network and an offline-network.
The online-network is updated using standard gradient descent, while the offline network is updated using the exponential moving average \Gls{EMA} of the online network.
...
Note that predicting the marginal velocity for \Gls{CFM} is significantly harder in the middle of the schedule than at the endpoints.
...
Similarly, at $t=t_0$ in \Gls{EDM}, $f_\theta$ should be the identity function, and at $t=t_\text{max}$, the ideal denoiser returns the mean of the training set.
...
For \Gls{EDM}, $\log(t) \sim \mathcal{N}(-1.2, 1.2)$ was chosen after a hyperparameter search and for \Gls{CFM} it was found that logit-normal sampling,
\begin{equation}
...
However, in the projects used throughout this thesis, optimal performance was found using a basic \Gls{CosEnc}~\cite{ImplicitQuantileNetworks}.
...
For parameters $t_\text{min}$, $t_\text{max}$, and $d$ the \Gls{CosEnc} layer is given by,
\begin{equation}
...
Gradients with respect to this model can be used to modify the update rule for the \Gls{ODE} or the velocity field to perform conditional generation.
This is often referred to as \textit{guidance} in the diffusion literature~\cite{DiffusionBeatsGANS}.

\subsubsection{Sampling}

As previously discussed, various numerical integration techniques can be employed with either the reverse \Glspl{SDE} or the probability flow \Gls{ODE} to sample from a diffusion model.
Beyond the conventional numerical integration methods, such as Euler's, Heun's, and Linear Multi-Step methods~\cite{heun}, a novel domain has emerged, focusing on developing efficient integration methods tailored to specific diffusion schedules.
