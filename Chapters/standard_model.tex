...
To this end, the \Gls{SM} of particle physics stands as the one of the most successful theoretical frameworks in the history of physics.
The \Gls{SM} is a \Gls{QFT} that encompasses three of the four fundamental forces of nature: electromagnetic, weak, and strong.
The fourth and final force, gravity, is not included in the \Gls{SM}\footnote{Gravity is $10^{37}$ times weaker than the weak nuclear force, and thus is negligible at the energy scales probed by particle accelerators, but can be incorporated to the theory through a coupling to a curved space-time.}.
The development of the \Gls{SM} was an iterative and collaborative effort that took much of the 20th century, during a time which saw great strides in the available technology, experimental techniques, and theory.
No experimental evidence has been found to contradict the \Gls{SM} predictions, much to the chagrin of many physicists who are eager to find the next breakthrough.

\section{Particle Content}

The \Gls{SM} describes the universe using only a handful of fundamental fields, the excitations of which are the particles we observe.
...
There are 12 fermions in the \Gls{SM}, divided into two categories: leptons and quarks.
...
The exceptions to this rule are the neutrinos which are assumed to be massless in the \Gls{SM} and are observed to oscillate between the three generations~\cite{SearchNeutrinosSun, NeutrinoOsc}.
...
Fermions also have distinct left- and right-handed chiral states which are treated differently in the \Gls{SM}\@.
A list of the fermions and their properties can be found in \Cref{tab:fermions}.
...
Forces between fermions in a \Gls{QFT} manifest via the exchange of spin-1 particles called vector- or gauge-bosons.
...
Unlike the other bosons in the \Gls{SM}, the bosons of the weak force have non-zero mass, resulting in a short range of interaction.
...
The last particle in the \Gls{SM} is the Higgs boson, a scalar particle with spin-0 which couples to all massive particles.
...
The \Gls{SM} is a gauge theory, a type of \Gls{QFT} which is symmetric under certain local transformations of a Lie group.
...
The Lagrangian of the \Gls{SM} is symmetric under the global Poincaré group, $\text{SL}(2, \mathbb{C})$, which encompasses the symmetries of special relativity.
...
In a \Gls{QFT}, all fundamental objects are described by quantum fields which are themselves functions of spacetime $x = (t, \vec{x})$.
...
The Lie group of local gauge transformations observed by the \Gls{SM} Lagrangian can be expressed as the product of three simple groups,
\begin{equation}
    \label{eq:sm_group}
...
The full Lagrangian of the \Gls{SM} is given by
\begin{equation}
    \label{eq:sm_lagrangian}
    \mathcal{L}_\text{\Gls{SM}} =
    \overbrace{
        \vphantom{\frac{1}{4}}
...
This is because particle masses in the \Gls{SM} are generated through interactions with the Higgs field $\phi$.
...
In the \Gls{SM} there are also three important discrete symmetries.
...
In the \Gls{SM} it is now known that the weak force can violate any combination of $\mathcal{C}$, $\mathcal{P}$, and $\mathcal{T}$, but not all simultaneously.
...
Fermi's theory of $\beta$-decay~\cite{Fermi1934} showed that it was possible to describe annihilation and creation of particles using \Gls{QFT}\@, solidifying it as an extremely useful framework for particle physics.
...
With this new technique, \Gls{QED}~\cite{AlexQED1,AlexQED2,AlexQED3} was fully established as a complete description of the electromagnetic force.
...
This results in the \Gls{QED} Lagrangian density,
\begin{align}
...
In the \Gls{SM}, $\mathcal{L}_\text{QED}$ is absorbed into the electroweak Lagrangian density, described in \Cref{sec:electroweak}.
...
The theory of quarks and their interactions via the strong force was formulated as a renormalizable \Gls{QFT} called \Gls{QCD} in the early 1970s~\cite{QCD1,QCD2,QCD3}.
...
\begin{equation}
    ...
    \end{array}
\end{equation}
The eight dimensional group corresponds to eight massless vector fields $G_\mu^a$ which are the gluons and, together with the strong coupling constant $g_s$, they yield the \Gls{QCD} covariant derivative,
\begin{equation}
...
The \Gls{QCD} Lagrangian density is then given by
\begin{equation}
...
An observed effect of \Gls{QCD} is \textit{colour confinement} where quarks and gluons are never observed in isolation, but are always bound together in colour-neutral ($\text{SU}(3)$ invariant) states called hadrons.
...
A consequence of confinement is hadronization, the process by which high-energy coloured particles freeze out into colour-neutral hadrons, which takes place during jet formation (\Cref{sec:jets}).
...
Around three decades later the \Gls{GSW}~\cite{Glashow1961, Weinberg1967,Salam1964} was developed to unify the electromagnetic and weak forces using the combined $\text{SU}(2)_L \times \text{U}(1)_Y$.
...
The effect of the charged weak interaction is therefore to rotate these doublets, effectively changing the flavour of the fermion\footnote{Since there is no other force through which the neutrinos can interact, there are no right-handed neutrinos in the \Gls{SM}.}.
...
The down-type quarks by convention mix between the weak eigenstates ($d', s', b'$) and the mass eigenstates ($d, s, b$) through the \Gls{CKM} matrix~\cite{cabibbo1963, CPViolationRenormalizableTheory},
\begin{equation}
    \label{eq:ckm_matrix}
...
\end{equation}
where the numerical values of the elements are determined experimentally~\cite{ParticleDataGroup}\footnote{The non-unitary nature of experimental values of the \Gls{CKM} matrix is in tension with the \Gls{SM} by 2.2$\sigma$}.
For the leptons, mixing is described with respect to the neutrinos and the \Gls{PMNS} matrix~\cite{Pontecorvo:1957qd, PMNSMatrix}.
...
\Gls{GSW} theory predicts 4 gauge fields $W^a_\mu$ and $B_\mu$ which do not correspond to the physical $W^\pm$, $Z$, and $\gamma$ bosons.
...
These discrepancies are resolved by the Brout-Englert-Higgs mechanism~\cite{Higgs1, Higgs2, Higgs3} (referred to hereafter as the Higgs mechanism), and the spontaneous breaking of the electroweak symmetry group.
...
Experimental evidence for the electroweak theory first came with the
observation of neutral current interaction with neutrinos in 1973\@~\cite{ObservationNeutrinolikeInteractions}, but the full discovery of the W and Z bosons at CERN would not come until 1983\@~\cite{WZ}.
Arguably the last major breakthrough in the field of particle physics was the discovery of the Higgs boson in 2012 by the \Gls{ATLAS} and \Gls{CMS} collaborations~\cite{ATLASHiggs, CMSHiggs}.

Consider a new scalar field given by a complex doublet,
\begin{equation}
    \phi = \begin{pmatrix} \phi^+ \\ \phi^0 \end{pmatrix} = \frac{1}{\sqrt{2}} \begin{pmatrix} \phi_1 + i \phi_2 \\ \phi_3 + i \phi_4 \end{pmatrix},
\end{equation}
embedded in a theory which observes the local $\text{SU}(2)_L \times \text{U}(1)_Y$ symmetry of the \Gls{GSW} model.
...
The expected form of the symmetry breaking should result in the group of \Gls{QED},
\begin{equation}
...
In \Gls{EWSB}, the broken symmetry should result in three massless Goldstone bosons, however there is no experimental evidence for such massless scalar particles.
...
\begin{equation}
    ...
    m_H = \sqrt{2 \lambda} v.
\end{equation}
For convenience, the bottom row of \Cref{eq:higgs_lagrangian_full} includes the dynamics and self-couplings of the electroweak fields in their original form before \Gls{EWSB}.
...
The free parameters of the \Gls{GSW} + Higgs mechanism are the electroweak coupling constants $g_W$ and $g_Y$, the Higgs self-coupling $\lambda$, and the vacuum expectation value $v$, all of which are determined experimentally.

The final piece of the \Gls{SM} is the generation of fermion masses through the Higgs mechanism.
...
A common feature in high-energy collisions, and a key object of study in much of this thesis, jets are emergent phenomena that arise due to the nature of \Gls{QCD}\@.
...
Jets are therefore formed during the transition from the perturbative to the non-perturbative regimes of \Gls{QCD}.
...
These conditions exist for all gauge theories, but are particularly important for \Gls{QCD} due the strength of $\alpha_s$ even at high energies.
...
If the quarks are produced with enough energy, this process repeats until the energy is low enough that the stable colour-neutral bound states emerge as shown by \Cref{fig:hadronization}.
...
Predictions made by the \Gls{SM} have been confirmed to an extraordinary degree of precision in collider experiments, as shown by \Cref{fig:sm_precision}.
Despite this, there are several reasons to believe that the \Gls{SM} is not the final theory of particle physics.
Many perceived shortcomings of the \Gls{SM} lead to open questions, active areas of research, and proposed extensions to the theory.

For example, gravity is a notable omission from the \Gls{SM} owing to the lack of a renormalizable \Gls{QFT} for gravity.
...
\begin{figure}
    ...
    \caption{Comparison of the total production cross-sections of various processes predicted by the \Gls{SM} and measured by the \Gls{ATLAS} experiment \cite{ATLAS:2024cgh}.}
...
% There is no candidate in the \Gls{SM} that fits this description.
% Furthermore, in the $\Lambda$CDM model of cosmology the dominant energy in the universe is \textit{dark energy}~\cite{LCDM1, LCDM2, LCDM3, LCDM4}, a positive pressure of the vacuum causing the universe to expand at an accelerating rate for which the \Gls{SM} offers no explanation.

In the \Gls{SM}, anti-matter is treated as a mirror image of matter.
...
CP violation is present in the \Gls{SM} from the \Gls{CKM} and \Gls{PMNS} matrices, but it is not sufficient to explain the observed matter-antimatter asymmetry.
It is supposed that there exists new physical processes beyond the \Gls{SM} that induces the much larger CP violation required for baryogenesis.

The above issues are associated with the \Gls{SM}'s inability to describe cosmological phenomena.
An additional set of ``issues'' are derived from aesthetic or philosophical considerations.
The \Gls{SM} has 19 free parameters that must be determined experimentally, a value considered by many to be too large to be considered a fundamental theory.
Another issue is the \textit{hierarchy problem}.
In a \Gls{QFT}, physical observables are determined by the sum of all possible Feynman diagrams, which often includes divergent terms.
...
Finally, the unification of \Gls{QED} and the weak force led many physicists to develop \Gls{GUTs}~\cite{GUT1, GUT2} which predict that all forces in the \Gls{SM} are unified at high energies.
Many of the predictions of \Gls{GUTs}, such as proton decay, have not been observed, and the energy scale at which the forces unify is far beyond the reach of current experiments.
