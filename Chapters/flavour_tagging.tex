...
We perform a grid search to test whether each of the proposed additions to the GNN design proposed in \Cref{sec:gnnp} are beneficial.
...
We begin by comparing the performance on the $\ttbar$ sample.
...
As shown in \Cref{tab:inference}, the GPU times are relevant for training the model, highlighting the ease with which these models can be produced. For offline analysis, taggers will be exported via ONNX runtime and run on the CPU\@.
