...
This chapter describes the design and implementation of several new \Glspl{GNN} and transformers for flavour tagging.
Jet flavour tagging is a crucial feature in many jet-based analyses at \Gls{ATLAS}, and an overview is provided in \Cref{sec:flavour_tagging}.
\Gls{ATLAS} uses dedicated algorithms to identify a jet's flavour, which corresponds to the flavour of the quark that initiated the jet, by analysing the properties of the associated charged particle tracks.
...
The previous state-of-the-art flavour tagger used by the \Gls{ATLAS} experiment was \Gls{GN1}, a GNN that uses the GATv2 system of message passing~\cite{GATv2}.
In this section, two new models and their variants are introduced.
Each are \Glspl{GNN} with different architectures and mechanisms for message passing.
The first new model is called \Gls{GN++} and is based on the complete GN Block architecture described in \Cref{sec:gn_block}, complete with persistent edge information and global attributes.
The second new model, Spice, is based on the transformer-encoder architecture described in \Cref{sec:transformers}.
Spice would turn out to be so performant and efficient that it would form the basis of the new GN2 flavour tagger adopted by the \Gls{ATLAS} collaboration~\cite{GN2Plots}.
...
Spice was being developed and presented to the flavour tagging group while \Gls{GN1} was still undergoing calibration.
...
It ensures that the models used by the \Gls{ATLAS} collaboration remain up-to-date with the state-of-the-art models employed by the broader machine learning community.

\section{Datasets}

Training and evaluating \Gls{GN++} and Spice requires the use of simulated datasets in order to provide a ground truth for the jet flavour.
This work reuses the same samples as the previous flavour taggers, DIPS and \Gls{GN1}, and further information can be found in \textcite{AlexThesis} and Ref.~\cite{GN1}.
...
The detector response is performed using the \geant simulation package~\cite{Geant4} with the \Gls{ATLAS} simulation setup~\cite{ATLASSim}.
...
The simulation output is reconstructed using the standard \Gls{ATLAS} reconstruction procedure described in \Cref{sec:event_reconstruction}.
...
As with \Gls{GN1}, these networks are trained to perform three complementary tasks.
...
\begin{table}
    ...
    \begin{tabular}{rl}
        ...
        7     & From a pileup \Gls{pp} interaction                                \\
...
The inputs to all models presented in this chapter are the same as those used in \Gls{GN1}.
...
All proposed models in this chapter can be considered as extensions of \Gls{GN1}.
Both Spice and \Gls{GN++} adhere to a similar framework as \Gls{GN1}, but incorporate advancements in message passing techniques and contemporary architectural trends.
...
\Gls{GN1} introduced the idea of a monolithic graph neural network which would simultaneously perform jet tagging, track identification, and vertexing matching.
...
To compare the information flow within the layers of \Gls{GN1}, the \Gls{GN++} models, and Spice, the flowcharts in \Cref{fig:gn1_graph}, \Cref{fig:gnpp_graph}, \Cref{fig:gna_graph}, and \Cref{fig:spice_graph} are used.
...
The specifics of the \Gls{GN1} model are described to better highlight the changes introduced in the \Gls{GN++} and Spice models.
Here, the original \Gls{GN1} implementation~\cite{GN1} is used, which is distinct from later variants~\cite{SamThesis} that adopted transformer-like features partly as a result of the success of Spice.
A diagram of the \Gls{GN1} model is shown in \Cref{fig:gn1}.
...
\Gls{GN1} uses three of these GATv2 layers to build its central feature extractor.
...
In total \Gls{GN1} has 820k trainable parameters with many of them being located in these three output MLPs and the track initializer.
The model was implemented using the PyTorch Geometric library~\cite{PYG}.

A schematic diagram of the GATv2 layers that make up the \Gls{GN1} tagger is shown in \Cref{fig:gn1_graph} which highlights some possible inefficiencies.
...
Finally, there is little to prevent oversmoothing in the node features, which may indicate why \Gls{GN1} was limited to three layers and was fairly reliant on the auxiliary tasks to improve performance.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.99\textwidth]{Figures/cern_atlas/GN1.png}
    \caption{Schematic diagram of the \Gls{GN1} model from Ref.~\cite{GN1}.}
    \label{fig:gn1}
\end{figure}
...
\begin{figure}
    ...
    \caption{Diagram of the operations within a GATv2 layer~\cite{GATv2} used by \Gls{GN1}.}
    \label{fig:gn1_graph}
\end{figure}
...
The \Gls{GN++} model is based on the complete GN Block architecture described in \Cref{sec:gn_block}.
...
This model is designed to be a more expressive graph network than \Gls{GN1}.
While each layer of \Gls{GN1} essentially operates on a set $\mathcal{N}$, \Gls{GN++} operates on a graph $(\mathcal{E}, \mathcal{N}, \u)$ with fully defined nodes, edges, and global attributes.
...
\begin{figure}[h!]
    ...
    \caption{Diagram of the operations within layer of the \Gls{GN++} tagger.}
    \label{fig:gnpp_graph}
\end{figure}

\begin{algorithm}[h!]
    \caption{The full \Gls{GN++} block. All weight calculations are followed by a softmax operation and square brackets denote concatenation.}
    \label{alg:gnpp}
    \begin{algorithmic}[1]
...
A basic linear projection is used for the track initializer instead of the full MLP used in \Gls{GN1}.
...
Five graph layers are used in total, each with half the embedding dimension of \Gls{GN1}, resulting in roughly 780k trainable parameters, slightly less than \Gls{GN1}.

Each of the features of the \Gls{GN++} model can be deactivated to investigate their impact on performance.
The minimal model, which we call GNA, is shown in \Cref{fig:gna_graph}, and it is close to, but not exactly the same, as \Gls{GN1}.
...
Like \Gls{GN++}, the model is equipped with MLPs for strong node updates, which double the hidden dimension of the nodes.
...
Like \Gls{GN1}, three separate MLPs perform the graph-, node-, and edge-level tasks.
These three MLPs and the track initializer are much smaller than the ones used in \Gls{GN1}, which reduces the number of parameters in the model.

To ensure a fair comparison, Spice has roughly the same number of trainable parameters as \Gls{GN1}.
...
The model is implemented using standard PyTorch operations and is thus conceptually easier to work with than both \Gls{GN++} and \Gls{GN1}.
...
The same training loss as \Gls{GN1} is used for all models, involving a weighted sum of three loss terms,
\begin{equation}
...
For the main models, the same loss coefficients as \Gls{GN1}, $\alpha = 0.5$ and $\beta = 1.5$.
...
The standard WPs for $b$-tagging algorithms in \Gls{ATLAS} are at 60\%, 70\%, 77\%, and 85\%, while the WPs for $c$-tagging are considerably lower, with a WP of 25\% being a common choice.
...
The standard \Gls{GN++} and Spice models are designed to have roughly the same number of trainable parameters as \Gls{GN1}.
...
Three additional networks are introduced: \Gls{GN++}large, Spice-large, and Spice-huge.
...
\Gls{GN++}large also has an embedding dimension of 128 and has 1.6M trainable parameters\footnote{Note that the node update MLPs within each block always doubles the embedding dimension.}.
...
The two extremes are the minimal model, GNA, which has all features deactivated and is similar in design to \Gls{GN1}, and the maximal model, \Gls{GN++}, which has all features activated.
...
Notably, all features are beneficial, and the best performance is achieved by the maximal model \Gls{GN++}.
...
However, as these models are limited to only three layers to match \Gls{GN1}, the benefits are less pronounced.
...
For each feature, four models are compared: GNA, GNA with the feature activated, \Gls{GN++}, and \Gls{GN++} with the feature deactivated.
The results for the four most important features are shown in \Cref{fig:ablation}.
The \Gls{GN++} tagger is the best-performing model for this sample, with around a 25\% increase over GNA in both light and $c$-jet rejection at $\bnom$, though the performance gain is less pronounced at higher signal efficiencies.
...
% After many iterations it is believed that the model design in \Gls{GN++} is well-optimized for the task at hand.
...
\begin{figure}[ht]
    ...
    \caption{ROC curves for the $b$-tagging discriminants using select \Glspl{GNN} from the hyperparameter grid search on $\ttbar$ jets with $20 < \pt < 250~\GeV$.
...
Quantitative comparisons between the new networks and \Gls{GN1} are conducted using their $b$- and $c$-tagging performance on the test $\ttbar$ and $Z'$ datasets.
...
\begin{table}[ht]
    ...
    \begin{tabular}{l@{\alen}cc@{\blen}cc@{\alen}cc@{\blen}cc}
        ...
        \Gls{GN++}  & 34                                   & 19                                  & 10          & 22          & 46          & 22          & 26          & 25          \\
        GN++large   & 59                                   & 46                                  & 21          & 42          & 65          & 41          & 37          & 50          \\
        \bottomrule
    \end{tabular}
    \caption{The percentage improvement over \Gls{GN1} in the background rejection rates at the nominal $b$-tagging and $c$-tagging working points using the $\ttbar$ and $Z'$ test datasets. The rejection rates are measured for the light-jet background $r_l$, the $c$-jet background $r_c$, and the $b$-jet background $r_b$.}
    \label{tab:comparison}
\end{table}

On the $\ttbar$ sample, all new \Glspl{GNN} outperform \Gls{GN1} at all tasks at all efficiencies.
...
The smaller networks, \Gls{GN++} and Spice, outperform \Gls{GN1} at $\bnom$ for both $b$- and $c$-tagging.
Spice also matches or exceeds \Gls{GN++} significantly in separating $b$ and light-jets, which is noteworthy as Spice lacks the feature determined to be the most important in the grid search, namely the persistent edge information.
It also is composed of significantly simpler layers.
Given the extended training period, ten times longer than the grid search, it is plausible that the transformer efficiently acquires the same critical information as the more complex \Gls{GN++}, albeit through a simpler architecture.
The larger models extend this performance gain, with Spice-huge offering a 73\% improvement in $r_l$ and 63\% improvement in $r_c$ over \Gls{GN1} at $\bnom$.
For $c$-tagging, Spice-huge remains the top performer, achieving a 26\% improvement in $b$-jet rejection and a 50\% improvement in light-jet rejection over \Gls{GN1} at $\cnom$.

On the $Z'$ sample, all new taggers once again outperform \Gls{GN1}.
Interestingly, for this sample, \Gls{GN++} outperforms the equivalently sized Spice.
Jets in this dataset have roughly an order of magnitude higher $\pt$ than the previous one and on average many more tracks.
Some features in the \Gls{GN++} architecture are evidently better suited to this data type.
However, the transformers demonstrate stronger scaling capabilities, with clear improvement with each tier in model size.
Spice-large is on par with \Gls{GN++}large, and Spice-huge is the top performer.
It achieves an improvement over \Gls{GN1} of 41\% in light-jet rejection and 65\% in $c$-jet rejection at $\bnom$.
...
\begin{figure}[ht]
    ...
    \caption{ROC curves for the $b$-tagging (left) and $c$-tagging discriminants (right) for $\ttbar$ jets with $20 < \pt < 250~\GeV$ (top) and $Z'$ jets with $\pt > 250~\GeV$ (bottom).
        All new models are compared to the benchmark \Gls{GN1}. The rejection values are calculated separately for each background type and the bottom two panels show the ratio to \Gls{GN1}. Error bands are derived from binomial uncertainties.}
    \label{fig:large_rocs}
\end{figure}

\subsection{Auxiliary Task Ablation Studies}

An investigation to determine whether the transformer continues to benefit from the two auxiliary tasks, that proved highly advantageous in \Gls{GN1}, is conducted.
To this end, three variants of the Spice-large model are trained, adjusting $\alpha$, $\beta$, or both to zero.

The results, as depicted in \Cref{fig:auxiliary}, indicate that while the auxiliary tasks are not as critical as in \Gls{GN1}, they do confer a marginal performance boost on average.
...
All CPU times are measured after the model has been exported to ONNX runtime~\cite{onnxruntime}, which is the tool used by the \Gls{ATLAS} collaboration for running deep learning models in their analysis framework.
...
The \Gls{GN++} models are the slowest on both CPU and GPU, likely due to the inefficient implementation of the graph layers in the custom graph library.
...
Despite the higher theoretical expressivity of a single \Gls{GN++} layer compared to a transformer layer, the efficiency of the latter allows it to scale more effectively.
...
For both \Gls{GN1} and \Gls{GN++}, significant effort is spent creating adaptation functions to convert the PyTorch Geometric layers or the layers from the custom graph library into ONNX-compatible operations.
...
\begin{table}[h!]
    ...
    \begin{tabular}{lrrr}
        ...
        \midrule
        \Gls{GN1}       & 820k       & 0.06          & 1.4           \\
        \Gls{GN++}      & 780k       & 0.50          & 23.0          \\
        \Gls{GN++}large & 1.6M       & 0.90          & 59.0          \\
        Spice           & 800k       & 0.03          & 1.5           \\
...
Developed in the \texttt{Salt}~\cite{Salt} framework, Spice was adapted into the new GN2 model, the new recommended flavour tagger actively used by the \Gls{ATLAS} Collaboration.
...
Compared to \Gls{GN1} the improvements are around $50-80\%$.
ROC curves comparing \Gls{GN1}, GN2, and deep sets based DL1d~\cite{DL1D}  are shown in \Cref{fig:gn2}.

\begin{figure}
    \centering
    \includegraphics[width=0.75\textwidth]{figures/flavour_tagging/year_improvement.pdf}
    \caption{The $c$- and light-jet rejection of the different \Gls{ATLAS} flavour tagging algorithms over time in MC simulated $\ttbar$ jets. The rejections are provided at the $\bnom$. The improvement with respect to the DL1 rejection is marked on each bar. Between DL1r and DL1d, a transition from Run 2 to Run 3 reconstruction took place. Plot taken from Ref.~\cite{GN2Plots}.}
...
\begin{figure}[ht]
    ...
    \caption{ROC curves for the $b$-tagging (left) and $c$-tagging discriminants (right) for $\ttbar$ jets with $20 < \pt < 250~\GeV$ (top) and $Z'$ jets with $250 < \pt < 6000~\GeV$ (bottom). The \Gls{GN1} and GN2 are compared to the baseline DL1d tagger with the bottom two panels showing the ratio of the rejection rates. Error bands are derived from binomial uncertainties. Plots are taken from Ref.~\cite{GN2Plots}.}
    \label{fig:gn2}
\end{figure}

\subsection{Further utilization of GN2 and Salt}

The GN2 model, alongside the \texttt{Salt} framework, has been extensively utilized across various analyses and projects within the \Gls{ATLAS} Collaboration.
...
The GN2 model and \texttt{Salt} framework have been incorporated into numerous ongoing works within \Gls{ATLAS}, including $\tau$-jet identification, vertex regression, electron/gamma energy calibration, pileup rejection, multi-top analyses, and lepton tagging.
...
The vertexing task, unchanged from \Gls{GN1} and based on~\textcite{SecondaryVertexFinding}, currently uses concatenated embeddings and an MLP for vertex prediction.
